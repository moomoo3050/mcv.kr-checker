async function getDNS(domain, type, service){
    if (type === "A") {
        ...
        fetch(`https://1.1.1.1/dns-query?name=_${service}._tcp.moomoo3050.mcv.kr&type=SRV`, {headers: {"accept": "application/dns-json"}})
        .then(response=>response.json())
        .then(data=>{ console.log(data.Answer[0].data.split(' ').at(-1)); })
    } else {
        alert("오류")
    }
}
